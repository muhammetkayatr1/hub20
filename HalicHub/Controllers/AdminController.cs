...
{ 
    [Authorize(Roles ="admin")]//BURAYA MUTLAKA YETKİLENDİRİLMİŞ GİRER DEMEK
    public class AdminController : Controller
    {
        ...
        public IActionResult Email(EMailModel m)  //Mail sınıfından m diye bir değişken tanımlarız
        {
            try
            {
                ...
                client.Credentials = new NetworkCredential("buraya kendi mailini yaz abi", "Buraya şifreni yaz");//<<<<<<<<<<<<<<<<<<<
                client.EnableSsl = true;
                MailMessage msj = new MailMessage(); //Yeni bir MailMesajı oluşturuyoruz
                msj.From = new MailAddress(m.Email, m.Name + " " + m.Surname); //iletişim kısmında girilecek mail buaraya gelecektir
                msj.To.Add("Buraya kendi mailini yaz abi"); //Buraya kendi mail adresimizi yazıyoruz <<<<<  buralar boş ondan sıkıntı çıkıyo abi doldurunca çalışacak. Varmı başak birşey abi yine problem olursa bakarız ?
                //Bu kısımdan itibaren sizden kullanıcıya gidecek mail bilgisidir 
                MailMessage msj1 = new MailMessage();
                msj1.From = new MailAddress("buraya kendi mailini yaz abi", "Haliç TeknolojiPortalı");//<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        public IActionResult NewsEmail(EMailModel m)  //Mail sınıfından m diye bir değişken tanımlarız
        {
            try
            {
                ...
                MailMessage msj = new MailMessage(); //Yeni bir MailMesajı oluşturuyoruz
                msj.From = new MailAddress(m.Email, m.Name + " " + m.Surname); //iletişim kısmında girilecek mail buaraya gelecektir
                msj.To.Add("buraya kendi mailini yaz abi"); //Buraya kendi mail adresimizi yazıyoruz<<<<<<<<<<<<<<<<<<
                //Bu kısımdan itibaren sizden kullanıcıya gidecek mail bilgisidir 
                MailMessage msj1 = new MailMessage();
                msj1.From = new MailAddress("buraya kendi mailini yaz abi", "Haliç Teknoloji Portalı");//<<<<<<<<<<<
                msj1.To.Add(m.Email); //Buraua iletişim sayfasında gelecek mail adresi gelecktir.
                msj1.Subject = "Haliç Hub Aboneliği";
                msj1.Body = "Haliç Hub Teknoloji Portalına Aboneliğiniz Gerçekleşmiştir. Aramıza Hoşgeldiniz...";
                client.Send(msj1);
        ...
        { var s = m.Subject;
            ...
            {//hata veriyosa güncellemedeki gibi yapsam
                msj.From = new MailAddress(item.Email, item.Name + " " + item.Surname); //iletişim kısmında girilecek mail buaraya gelecektir
                msj.To.Add("buraya kendi mailini yaz abi");// <<<<<<<<<<<<<<<<<<<<<<< 
                MailMessage msj1 = new MailMessage();
                msj1.From = new MailAddress("buraya kendi mailini yaz abi", "Haliç Teknoloji Portalı"); //<<<<<<<<<<<<<<<<<
